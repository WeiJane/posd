...
TEST(Shell2, exception_OnlyVariable) {
...
TEST(Shell2, exception_UnexpectedDisjTokenBeforePeriod) {
  Scanner s("X;.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell2, exception_UnexpectedConjTokenBeforePeriod1) {
  Scanner s("X,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell2, exception_UnexpectedConjTokenBeforePeriod2) {
  Scanner s("X=1,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell2, exception_UnbalancedOperator1) {
...
TEST(Shell2, exception_UnbalancedOperator2) {
